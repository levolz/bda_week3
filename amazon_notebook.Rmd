...
features_df$n_words <- unname(features_df$n_words)
features_df$n_sentences <- unname(features_df$n_sentences)
features_df$prop_stopwords <- unname(features_df$prop_stopwords)
features_df$n_swear_words <- unname(features_df$n_swear_words)
features_df$n_bad_words <- unname(features_df$n_bad_words)
features_df$n_neg_words <- unname(features_df$n_neg_words)
features_df$n_pos_words <- unname(features_df$n_pos_words)

features_df[,-c(1,3,4)] <- lapply(features_df[,-c(1,3,4)], as.numeric)

features_df <- features_df %>%
  mutate_at(c(2, 5:31), ~replace(., is.na(.), 0))

library(Matrix)
library(Seurat)
X2 <- as.sparse(features_df[, -c(1,3,4)])
cbind(X, X2)
# Target variable
...
model_binary <- glmnet(train_set, as.factor(I(target > 3)), family = "b", alpha=1, trace.it = TRUE, nlambda = 5, type.measure = "auc")

plot(model_interval)

cv_interval <- cv.glmnet(train_set, target, family = "g", alpha = 1, trace.it = TRUE)
cv_binary <- cv.glmnet(train_set, as.factor(I(target > 3)), family = "b", alpha = 1, trace.it = TRUE)
