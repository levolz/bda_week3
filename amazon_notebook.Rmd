...
<br>
1. The data came from reviews about baby products on Amazon. The results will generalize to the population who will buy baby products and write a review about it. 
<br>
2. The data we will work with is very big and therefore we can not do the feature selection by hand. We can use the Ridge or the Lasso regression  for automatic feature selection.
<br>
3. The Bayes error bound means how well humans can predict. In this case, a human would most likely to be able to accurately recognize if a customer is satisfied or not based on the review. We can argue that it’s around 95% accuracy.
<br>
...
Features computed for tokens in text are based on the Bag of Words (BoW) model: Each document is considered a bag of words, in which order plays no particular. A better name would be Bag of Tokens, because tokens can also be bi-grams etc., but we're stuck with BoW. Common features are
...
stopwords_df <- reviews %>%
    inner_join(stopwords, by = c(token = "word")) %>%
    group_by(id) %>%
    summarize(n_stopwords = n())

# Joining the features in dataframe. 
amazon_features <-
  inner_join(amazon, amazon_sent, by = "id") %>%
...
swear_wordsdf <- ifelse(reviews$token %in% swear_words, 1, 0)
sum(swear_wordsdf)

swear_words2 <- read.delim("http://www.cs.cmu.edu/~biglou/resources/bad-words.txt", header = FALSE)
swear_wordsdf2 <- ifelse(reviews$token %in% swear_words2, 1, 0)
sum(swear_wordsdf2)
