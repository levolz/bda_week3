...
rm(swear_words_df,bad_words_df, neg_words_df, pos_words_df, sentence_length_df, sent_scores, stopwords_df)
...
#Fit binominal logistic regression
...
```{r}
# Model comparisons
# Model comparisons (Credit to group 4)

# Performance evaluation
# For lasso
lasso_AUC <- max(cv_lasso$cvm)
lasso_acc <- mean(pred_lasso == y)

# For ridge
ridge_AUC <- max(cv_ridge$cvm)
ridge_acc <- mean(pred_ridge == y)

# For binominal logistic regression
model_binary_AUC <- max(cv_binary$cvm)
model_binary_acc <- mean(pred == y)

model_comp <- data.frame(models = c("Lasso", "Ridge", "Binary Log"),
                AUC = c(lasso_AUC, ridge_AUC, model_binary_AUC))

# Show the predictive performance measures
model_comp

model_comp <- model_comp %>% mutate(ToHighlight = ifelse( AUC == max(AUC), "yes", "no" ) )

row.names(model_comp) <- c('Lasso', "Ridge", "Binary Log")

ggplot(model_comp, aes(x= reorder(rownames(model_comp), -AUC), y = AUC, fill = ToHighlight, label = AUC)) +
geom_col() +
